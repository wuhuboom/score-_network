					...
					if($inplay){
						...
						if(empty($odds)){
							...
							if (!empty($data['results'])) {
								...
								$log_contents = '更新赛程数据：' . json_encode($save_data, JSON_UNESCAPED_UNICODE);
								LogHandler::getInstance()->log($log_contents, LogHandler::getInstance()::LOG_LEVEL_INFO, 'Odds');
						...
						$log_contents = '没有需要更新的赛程' ;
						LogHandler::getInstance()->log($log_contents, LogHandler::getInstance()::LOG_LEVEL_INFO, 'Odds');
					}
					\co::sleep(3);     	//每3秒获取一次
				}catch (\Throwable $e){
					$log_contents = "赛程自定义进程错误：{$e->getMessage()}_{$e->getLine()}_{$e->getCode()}";
					LogHandler::getInstance()->log($log_contents,LogHandler::getInstance()::LOG_LEVEL_INFO,'Odds');
