 ...
 */
class Vip extends \App\HttpController\Admin\Base
{
    /**
     * 产品列表
     */
    public function lists(){
        ...
        $data = VipService::create()->getLists($where,$field,$page,$limit,'level asc');
    ...
    public function add(){
        try{
            $data = $this->param;
            $data['require_buy_s'] = $data['require_buy']??0;
            $data['create_time'] = date('Y-m-d H:i:s');
            $data['update_time'] = date('Y-m-d H:i:s');
            $result = VipService::create()->validateData($data,'add');
            if($result!==true) {
                $this->AjaxJson(0,$data,$result);return false;
            }
            if(VipService::create()->getOne(['code'=>$data['code']])){
                $this->AjaxJson(0, [], 'VIP级别编码已存在');return false;
            }
            if($insert_id =  VipService::create()->save($data)){
    ...
    public function edit(){
        ...
        try {
            if (!empty($this->param['id'])) {
                ...
                $data['require_buy_s'] = $data['require_buy']??0;
                $data['update_time'] = date('Y-m-d H:i:s');
                $result              = VipService::create()->validateData($data, 'edit');
                if ($result !== true) {
                    $this->AjaxJson(0, $data, $result);
                    return false;
                }
                if(VipService::create()->getOne(['code'=>$data['code'],'id'=>[$this->param['id'],'<>']])){
                    $this->AjaxJson(0, [], 'VIP级别编码已存在');return false;
                }
                if (VipService::create()->update($this->param['id'],$data )) {
    ...
    public function del(){
        if(!empty($this->param['ids'])){
            ...
            if( VipService::create()->delete(['id'=>[$ids,'in']])){
     ...
     */
    public function all(){
        $list = VipService::create()->getLists([],'id as value, name',0,0,'level asc');
