...
class League extends \App\HttpController\Admin\Base
{
    ...
    public function lists(){
        ...
        $data = LeagueService::create()->getLists($where,$field,$page,$limit,'id desc');
		...
		try {
			...
			if($data['results']){
				foreach ($data['results'] as $k=>$v){
					...
					if($league = LeagueService::create()->getOne(['cc'=>$save_data['cc']??'','name'=>$save_data['name']])){
						LeagueService::create()->update($save_data['id'],$save_data );
					}else{
						LeagueService::create()->save($save_data);
					}
				}
    ...
    public function add(){
        try{
            ...
            $result = LeagueService::create()->validateData($data,'add');
            if($result!==true) {
                $this->AjaxJson(0,$data,$result);return false;
            }
            if(LeagueService::create()->getOne(['name'=>$data['name']])){
                $this->AjaxJson(0, [], '联赛名称已存在');return false;
            }
            if(LeagueService::create()->getOne(['cc'=>$data['cc']])){
                $this->AjaxJson(0, [], '联赛简称已存在');return false;
            }
            if($insert_id =  LeagueService::create()->save($data)){
    ...
    public function edit(){
        ...
        try {
            if (!empty($this->param['id'])) {
                $data                = $this->param;
                $data['update_time'] = date('Y-m-d H:i:s');
                $result              = LeagueService::create()->validateData($data, 'edit');
                if ($result !== true) {
                    $this->AjaxJson(0, $data, $result);
                    return false;
                }
                if(LeagueService::create()->getOne(['cc'=>$data['cc'],'id'=>[$this->param['id'],'<>']])){
                    $this->AjaxJson(0, [], '联赛简称已存在');return false;
                }

                if(LeagueService::create()->getOne(['name'=>$data['name'],'id'=>[$this->param['id'],'<>']])){
                    $this->AjaxJson(0, [], '联赛名称已存在');return false;
                }
                if (LeagueService::create()->update($this->param['id'],$data )) {
    ...
    public function del(){
        if(!empty($this->param['ids'])){
            ...
            if( LeagueService::create()->delete(['id'=>[$ids,'in']])){
     ...
     */
    public function all(){
        $list = LeagueService::create()->getLists([],'id as value, CONCAT("【",cc,"】",name) as name',0,0,'id asc');
