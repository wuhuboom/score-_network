...
class Service extends \App\HttpController\Admin\Base
{
    /**
     * 产品列表
     */
    public function lists(){
        ...
        if(!empty($this->param['code'])) {
            $where['WhatsApp'] = ["%{$this->param['WhatsApp']}%", 'like'];
        }
        ...
        $data = ServiceService::create()->getLists($where,$field,$page,$limit,'id asc');
    ...
    public function add(){
        try{
            ...
            $result = ServiceService::create()->validateData($data,'add');
            if($result!==true) {
                $this->AjaxJson(0,$data,$result);return false;
            }
            if($insert_id =  ServiceService::create()->save($data)){
    ...
    public function edit(){
        ...
        try {
            if (!empty($this->param['id'])) {
                ...
                $data['update_time'] = date('Y-m-d H:i:s');
                $result              = ServiceService::create()->validateData($data, 'edit');
                if ($result !== true) {
                    $this->AjaxJson(0, $data, $result);
                    return false;
                }

                if (ServiceService::create()->update($this->param['id'],$data )) {
    ...
    public function del(){
        if(!empty($this->param['ids'])){
            ...
            if( ServiceService::create()->delete(['id'=>[$ids,'in']])){
     ...
     */
    public function all(){
        $list = ServiceService::create()->getLists([],'id as value, name',0,0,'id asc');
