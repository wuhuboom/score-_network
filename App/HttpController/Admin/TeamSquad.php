...
use App\Service\TeamSquadService as Service;
use EasySwoole\HttpClient\HttpClient;

class TeamSquad extends \App\HttpController\Admin\Base
{
    /**
     * 产品列表
     */
    public function lists(){
	    ...
	    if(!empty($this->param['name'])) {$where['tq.name'] = ["%{$this->param['name']}%", 'like'];}
	    if(!empty($this->param['team_name'])) {$where['t.name'] = ["%{$this->param['team_name']}%", 'like'];}

	    $field = 'tq.*,t.name as team_name';
	    $page = (int)($this->param['page']??1);
	    $limit = (int)($this->param['limit']??10);
	    $data = Service::create()->joinSelectList($where,$field,$page,$limit,'tq.id desc');
    ...
    public function getDataByApi(){
	    try {
		    ...
		    $task->async(new \App\Task\TeamSquad([]));
