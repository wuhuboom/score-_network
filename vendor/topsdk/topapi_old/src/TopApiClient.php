...
class TopApiClient
{
    ...
    public function curl($url, $postFields = null)
    {
        ...
        $client->setHeader('User-Agent','new_php_sdk');
        if (is_array($postFields) && 0 < count($postFields))
        {
            ...
            //    $client->setHeader('content',' application/x-www-form-urlencoded; charset=UTF-8');
            $client->setContentTypeFormUrlencoded();
            $reponse = $client->post($postBodyString);
        }else{
            $reponse = $client->get();
        }
        if (200 !== $reponse->getStatusCode()) {
            throw new Exception($reponse,$reponse->getStatusCode());
        }
