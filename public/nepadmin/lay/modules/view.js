...
layui
  ...
  .define(
    ['jquery', 'laytpl', 'element', 'form', 'loadBar', 'dropdown', 'util', 'table'],
    function(exports) {
      var $ = layui.jquery
      var view_dropdown = layui.dropdown
      var util = layui.util
      ...
      self.tab = {
        ...
        init: function() {
          ...
          $(document).on('click', btnCls, function(e) {
            ...
            if ($(this).attr('id') == 'nepadmin-tabs-down') {
              view_dropdown.render({
                elem: '#nepadmin-tabs-down'
                , trigger: 'click'
                , click: function (data, othis) {
                  console.log(data)
                  data.id == 2 ? tab.delAll() : tab.delOther()
                }
                , data: [{
                  title: '关闭其他选项卡'
                  , id: 1
